...
from utils_rcnn import WEIGHTS_NAME, Config, cached_path, hf_bucket_url, is_remote_url, load_checkpoint
...
class Res5ROIHeads(nn.Module):
    ...
    def forward(self, features, proposal_boxes, gt_boxes=None):
        ...
        if gt_boxes is not None:
            gt_boxes = torch.Tensor(gt_boxes).view(len(proposal_boxes), -1, 4)
            for i, pb in enumerate(proposal_boxes):
                gb = torch.Tensor(gt_boxes[i]).cuda()
                proposal_boxes[i] = torch.cat((gb, pb), dim=0)
