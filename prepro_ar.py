    ...
    def __getitem__(self, index):
        ...
        if self.task == 'qa':
            ...
            for i, answer_choice in enumerate(answer_choices):
                ...
                factual_target = torch.Tensor([1]).long()
                out.append((torch.Tensor(tokenized), torch.Tensor(token_type_ids),
                            roi_feature, pos,
                            torch.Tensor(attention_mask), target, factual_target))
        else:
            # qar
            rationale_choices = self.data.rationale_choices[index]
            out = []
            for i, answer_choice in enumerate(answer_choices):
                a_str, _ = list_to_str_only(answer_choice, names)
                answer =  tokenizer(a_str, return_token_type_ids=False)
                for j, rationale_choice in enumerate(rationale_choices):
                    r_str, _ = list_to_str_only(rationale_choice, names)
                    ...
                    if i == answer_label:
                        ...
                        factual_target = torch.Tensor([0]).long()
                    out.append((torch.Tensor(tokenized), torch.Tensor(token_type_ids),
                                roi_feature, pos,
                                torch.Tensor(attention_mask), target, factual_target))
        return tuple(out)
...
def vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feat,
     img_pos, attn_masks, targets, factual_targets) = map(list, unzip(concat(inputs)))
    ...
    factual_targets = torch.stack(factual_targets, dim=0)
    ...
    batch = {'input_ids': input_ids.long(),
             ...
             'factual_targets': factual_targets.long()}
